...
async function _clean_boats() {
  ...
  for await (const boat of boats) {
    const time_offset =
      (new Date().getTime() - boat.locations[boat.locations.length - 1].TIMESTAMP.getTime()) /
      (1000 * 60 * 60)
...
async function _insert_or_update_current_boats(ais_data_list) {
  ...
  for (const ele of ais_data_list) {
    ...
    if (boat == null) {
      ...
      const last_timestamp = boat.locations[boat.locations.length - 1].TIMESTAMP
      if (last_timestamp.getTime() != _TIMESTAMP_to_date(ais_data.TIMESTAMP).getTime()) {
        console.log('New timestamp!')
        let new_locations = [...boat.locations, _get_new_location_obj(ais_data)]
...
function _predict_ship_movement(AIS, key, angle_offset = 0, distance = 50) {
  ...
  const lng1 = AIS['LONGITUDE'] * degToRad
  ...
  const lng =
    ((lng1 - Math.asin((Math.sin(tc) * Math.sin(d)) / Math.cos(lat)) + Math.PI) % (2 * Math.PI)) -
    Math.PI

  return [lat * radToDeg, lng * radToDeg]
}

function _get_predicted_ship_position(ais_data, hour_offset, degree_offset = 10) {
...
function _get_predict_cone(ais_data, distance = 50, cone_degrees = 10) {
  let ret_points = []
  ret_points.push(_predict_ship_movement(ais_data, 'COURSE', cone_degrees, distance))
...
// or {cone: [], line: [], center: []}
async function predict_boat_movement_range(
  boat_id,
  minutes_diff_start,
  minutes_diff_end,
  interval_minutes,
  current_minute_offset,
) {
  const collection = await db.collection('boats')
  const boat = await collection.findOne({ _id: boat_id })
  if (boat != null) {
    ...
    for (let i = minutes_diff_start; i <= minutes_diff_end; i += interval_minutes) {
      ...
      const interval_movement_predict = await predict_boat_movement_single(hour_offset, {
        last_ais_point: last_pos,
      })
      intervals.push({
...
async function serialize_boats(minute_range) {
  ...
  for (const boat of boats) {
    ...
    const minute_offset = (new Date().getTime() - last_location_time.getTime()) / (1000 * 60)
    let min_minutes = Math.floor(minute_offset - minute_range)
    const max_minutes = Math.floor(minute_offset + minute_range)
    if (min_minutes < 0) {
      min_minutes = 0
    }
    boat['prediction'] = await predict_boat_movement_single(minute_offset / 60, {
      boat_id: boat._id,
    })
    boat['prediction_range'] = await predict_boat_movement_range(
      boat._id,
      min_minutes,
      max_minutes,
      1,
...
export { update_boats, predict_boat_movement_range, gliders_in_boath_path, serialize_boats }
